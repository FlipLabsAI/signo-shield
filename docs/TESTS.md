...
`forge test` runs everything: 5 suites. The fork suites read public archive
...
| Fee | `test_fee_*` (core), `test_repay_takesTheLaunchFeeWhenARecipientIsSet` (X Layer) | Stamped at registration, taken only with a recipient, counted against the budget |
